...
func AnalyseParseUsersPost() usecase.Interactor {
	...
	u := usecase.NewInteractor(func(ctx context.Context, input Request, output *string) error {
		...
		inputErr = os.WriteFile(path.Join(utils.WorkDir("magic-spaces"), "userssample.json"), body, 0644)
		if inputErr != nil {
			return inputErr
		}

		_, err := execution.ExecutePowerShell("john", "*", "magic-spaces", "30-analyse", "10-parse-users.ps1", "")
		if err != nil {
			return err
		}
